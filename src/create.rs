...
pub fn create_file(path: &Path, size_bytes: u64) -> Result<()> {
    ...
    let file = io_backend::open_write(path)?;
...
pub fn create_file(path: &Path, size_bytes: u64) -> Result<()> {
    ...
    let mut file = io_backend::open_write(path)?;
    ...
    while records_remaining > 0 {
        let recs = (BUF_RECORDS as u64).min(records_remaining) as usize;
        fill_records(buf.as_mut_slice(), recs, &mut rng);
        let bytes = recs * RECORD_SIZE;

        file.write_all(&buf.as_slice_range(bytes))
